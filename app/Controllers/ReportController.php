...
HTML;
    ...
    public function qurbanSapiTemplatePrint(string $qurbanId): void
    {
        $pdfRoute = '/api/reports/qurban/' . rawurlencode($qurbanId) . '/template.pdf';
        $src = '/index.php?__route=' . rawurlencode($pdfRoute);
        $html = <<<HTML
<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="utf-8">
  <title>Print Kwitansi Qurban</title>
  <style>
    html, body {
      margin: 0;
      height: 100%;
...
<body>
  <iframe id="pdfFrame" src="{$src}"></iframe>
  <div id="printStatus" class="print-status">Menyiapkan preview kwitansi qurban...</div>
  <script>
    const frame = document.getElementById('pdfFrame');
    const statusBox = document.getElementById('printStatus');
    let printStarted = false;
    ...
    function runPrint() {
      ...
      window.addEventListener('focus', function onFocus() {
        setTimeout(function () {
          setStatus('Preview kwitansi qurban siap.', true);
        }, 300);
        window.removeEventListener('focus', onFocus);
      });
    }
    ...
    frame.addEventListener('load', function () {
      loadDetected = true;
      setStatus('Preview kwitansi qurban siap.', false);
      setTimeout(runPrint, 350);
    });

    setTimeout(function () {
      if (!loadDetected) {
        setStatus('Masih memuat PDF kwitansi qurban...', false);
      }
    }, 800);
...
HTML;
    ...
    public function qurbanSapiTemplatePdf(string $qurbanId): void
    {
        try {
            ...
            http_response_code(200);
            header('Content-Type: application/pdf');
            header('Content-Disposition: inline; filename="kwitansi-qurban-' . rawurlencode($qurbanId) . '.pdf"');
            echo $bytes;
            exit;
        } catch (RuntimeException $exception) {
            Response::json([
